...
\begin{algorithmic}[1]
\Procedure{solve\_2armed\_bandit\_dp}{$M, \gamma, tol, max\_iter$}
    \State \textbf{Input:}
    ...
    \For{$it = 1$ to $max\_iter$}
        \State $delta \gets 0$ \Comment{Track maximum change in this iteration}
        \For{$\alpha_1 = 1$ to $M$}
            \For{$\beta_1 = 1$ to $M$}
                \For{$\alpha_2 = 1$ to $M$}
                    \For{$\beta_2 = 1$ to $M$}
...
\includepdf[pages=-]{code/programming.pdf}  % Include all pages of file.pdf
